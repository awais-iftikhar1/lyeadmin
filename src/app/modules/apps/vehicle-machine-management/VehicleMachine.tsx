...
const VehicleMachine = () => {
  ...
  const filterData = async (filterType: string,pageSize: number, page: number) => {
    setIsLoading(true);
    try {
      const data = await getVehicleMachineType(filterType,pageSize, page - 1);
      console.log(data);
    ...
    <Routes>
      <Route element={<Outlet />}>
        <Route
          ...
          element={
            <>
              <PageTitle breadcrumbs={breadCrumbsData['Vehicle Machine']}>{routes[route as keyof typeof routes] }</PageTitle>

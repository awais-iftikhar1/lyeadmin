...
import { CorporateRequestListing } from '../../../../_metronic/partials/widgets/tables/CorporateRequestListing';
import { viewCorporateRequests } from '../../../api/Corporate';
...
const CorporateRequestDetails = () => {
  ...
  const filterData = async (pageSize: number, page: number) => {
    setIsLoading(true);
    try {
      const data = await viewCorporateRequests();
  ...
  return (
    ...
    <Route element={<Outlet />}>
      <Route
        path='corporateRequest'
        element={
          <>
          
            <PageTitle breadcrumbs={breadCrumbsData['Corporate Request']}>{routes[route as keyof typeof routes] }</PageTitle>

            
              <KTCard>
                  <KTCardBody className='py-4'>
                    <CorporateRequestListing
    ...
    <Route
      index
      element={<Navigate to='/apps/corporate-management/corporateRequest' />}
    />

 ...
 */
export const deletePackages = async (packageId: string): Promise<IUser> => {
  return new Promise(async (resolve, reject) => {
    const jwt: any = getAuth();

    try {
              // url: `${API_URL}admin-package/${packageId}`,
        // headers: {
        //   Authorization: jwt,
        // },
        debugger
      const { data } = await Axios({
        ...
        url: `${'http://135.148.40.91:2000/api/'}package/deletePackage`,
        headers: {

...
import FilterAddEdit from '../../../../app/modules/apps/filter-management/filters/filter-add-edit';
...
import { FiltersEnum } from '../../../../app/utils/enum/DropDowns';
import { IOptionValue } from '../../../../app/modules/apps/user-roles/roles-privileges-list/rolesPrivilegesModel';
import { deleteFilters } from '../../../../app/api/filters.ts';
import { Filter } from '../../../../app/modules/apps/filter-management/types';
type Props = {
  className: string;
  loading: boolean;
...
const FilterListing: React.FC<Props> = ({
  className,
  loading,
  setRefreshList,
  setTabIndex,
  ...
  const [listDetails, setListDetails] = useState<Filter|null>(null);
  ...
  const filterDetail = async (data: Filter|null) => {    
    try {
      setListDetails(data)
  ...
  const deleteFilterHandler = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await deleteFilters(itemId);
      console.log(data);
  ...
  return (
    <>
              ...
              <table className='table align-middle gs-0 gy-4'>
                ...
                <thead>
                  <tr className='fw-bold text-muted bg-light'>
                    <th className='ps-4 min-w-150px rounded-start'>Filter Type</th>
                    <th className='ps-4 min-w-200px rounded-start'>Manufacture Name</th>
                    <th className='min-w-200px'>Part No</th>
                    <th className='min-w-150'>Price</th>
      ...
      <ConfrimModal
        ...
        confirmProcess={deleteFilterHandler}
